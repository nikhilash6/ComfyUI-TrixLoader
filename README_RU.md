...
По двойному нажатию на вкладку **mask** или клику на иконку **Волшебной палочки** на панели управления открывается новый полноэкранный **Продвинутый AI Редактор масок**. Выход осуществляется по кнопке Exit или клавише Esc.

#### Доступные инструменты:
* **Обычное рисование**: Ручная кисть и ластик с плавной регулировкой размера и жесткости.
* **Выбор объектов (SAM)**: Активируйте инструмент SAM, выберите подходящую модель (Fast, Balanced или Best) и просто кликайте на объекты на картинке для автоматического выделения.
* **Удаление фона**: Мгновенно вырезайте объекты или удаляйте задний план с помощью нейросетей (`u2net`, `isnet` или `birefnet`).
* **Смещение и Размытие (Grow & Blur)**: Расширяйте границы маски и размывайте края в реальном времени с интерактивным отображением на холсте.
* **Полупрозрачность маски**: Настраивайте прозрачность маски на выходе ноды для мягкого смешивания краев.
* **Интеграция с Outpaint обрезкой**: Кликните **Pad Image for Outpaint** для перехода в Crop/Pad редактор, настройте расширение холста и вы автоматически вернетесь в редактор масок с сохраненным рисунком и точно выровненными координатами.
