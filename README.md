...
* **Segment Anything (SAM)**: Toggle the SAM tool, select a model, and click on an object to segment it.
  ...
  - **SAM PRO Mode**: When enabled, it focuses SAM on a dynamic square crop box representing your active zoomed viewport (marked with a dashed blue border and dimmed background). This dramatically increases segmentation accuracy for fine details (like hair) and ignores background elements outside the box. It also automatically post-processes the mask to eliminate small noise and isolate only the connected components containing your click points.
